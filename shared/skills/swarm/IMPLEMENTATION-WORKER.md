...
Bounded implementation slice in a coordinator-provided isolated worktree. Dispatched by `swarm`; see `SKILL.md` for the framing, dropout, and aggregation contract.

Work only on the standalone slice in the brief and only in the coordinator-provided working directory. Do not create worktrees, branches, ports, servers, or registries, and do not merge changes. Do not broaden the slice or modify unrelated files.
...
2. Run the narrowest relevant verification available in the provided working directory.
