...
1. **Create a private working-tree snapshot.** Create a mode-0700 temporary directory under `${PI_SESSION_TMPDIR:-${TMPDIR:-/tmp}}`. Capture `git status --short` exactly once in a separate mode-0600 regular file. Test that file with `-s`—never test command or process-substitution output directly. If it is empty, remove the temporary directory, report no work, and stop. Otherwise, write any user-stated scope, the captured status file's contents, `git diff` (unstaged), and `git diff --staged` to a mode-0600 snapshot file. Resolve `../_shared/commit-message-rules.md` from this skill directory; append its rules, the output of `../generate-commit-message/scripts/detect_commit_style.sh`, the output of `../_shared/scripts/git_ticket_key.sh`, and `git log -10 --format=%s`. For every nonignored untracked path, include its type and byte size; include regular-file contents as a diff against an empty temporary file. When appending that diff, treat exit statuses 0 and 1 as successful and propagate any higher status. Mark unreadable, nonregular, or binary files whose contents cannot be captured as requiring a warning and exclusion from automatic staging. Do **not** read the snapshot or emit the diff into the parent context. The command may report only whether the captured status file is empty.
...
4. **Surface non-blocking warnings with the plan.** If a ready plan reports a mixed-concern file, possible secrets, credentials, client data, generated material, uncaptured or nonregular untracked paths, or unrelated pre-existing staged changes, call `ask_question` with the warning and the complete numbered commit plan together in `details`. Show every group in commit order with its proposed message and exact paths—the user must not have to decide how to resolve a warning without seeing the plan it affects. Offer resolution options specific to the warning. Otherwise, use the returned groups and messages unchanged for the confirmation plan.
