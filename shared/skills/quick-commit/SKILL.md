...
2. **Delegate all diff analysis.** Dispatch exactly one cheap, read-only `commit-planner` subagent with the snapshot's absolute path — never put the full diff in the delegation prompt. The planner is the sole reader of the working-tree evidence and follows `COMMIT-PLANNER.md` in this skill directory. It returns either a ready commit plan or a blocking clarification. Retain the directory until the user accepts the final plan or cancels; the parent retains all staging, committing, pushing, and user confirmation.
