...
2. **Gather the evidence the explanation needs.** Inspect original source directly when explaining interconnected behavior; multi-file discovery does not require delegation. Use `bulk-read` for an independent factual extraction only when its answer can replace substantial reading. When delegating, request source-backed answers, unresolved facts, and coverage limits—not just a symbol map. Apply its use/inspect/disclose triage: use supported facts directly, inspect unresolved reasoning or conflicting evidence, and disclose gaps that need not be resolved for this explanation. Do not turn every cited source into a reading assignment. Small, already-localized questions stay direct.
...
3. **Track claims by kind.**
   - **Observed:** behavior, structure, or rationale explicitly supported by current code, tests, configuration, comments, history, tasks, documents, or review discussion. Cite a concrete `path:line` or `path:line-range`, symbol, commit, PR, task, or document. A worker's observed path and line range qualifies; do not reread the file to convert it into your own line number.
