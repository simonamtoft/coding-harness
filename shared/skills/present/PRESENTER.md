...
1. Assemble rather than hand-author the complete page. Choose one path:
   ...
   - Run `scripts/assemble-report.mjs` from this skill directory with the manifest's `outputFile` set to `${PI_SESSION_TMPDIR:-${TMPDIR:-/tmp}}/agent-final-<report-id>.html`. Overwrite only temporary artifacts carrying this report's ID on each turn.
