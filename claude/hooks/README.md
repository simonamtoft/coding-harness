...
**Hard-deny list.** `sudo`; pipe-to-shell (`… | sh`, but not `… | bash script.sh`); `git push --force`/`-f` (**not** `--force-with-lease`); `git reset --hard`; `git filter-branch`/`filter-repo`; `git clean -f`; `chmod 777`; `rm` provably outside `$PWD` / `/tmp` / `~/.claude/plans`; any segment touching a protected secret path.
