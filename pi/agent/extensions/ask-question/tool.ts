...
export const CLARIFICATION_GATE = `Clarification gate (mandatory): Read-only discovery comes first. Inspect the repository whenever files, tests, or conventions can resolve an uncertainty, and do not ask what that inspection answers. Before any consequential action, including edits, writes, commands with effects, and committing to an approach, decide whether unresolved choices remain that could materially change scope, behavior, user experience, data design, dependencies, destructive effects, or acceptance criteria. If they do, collect all closely related blocking questions into one ask_question call before acting. Inspection settles facts, not authorization: when a request that deletes, overwrites, or discards work leaves its scope ambiguous, confirm the scope before acting even if inspection suggests a plausible target. You MUST use ask_question for every such question; never ask it as ordinary assistant prose unless ask_question reports that interactive UI is unavailable, in which case ask in the assistant response and stop until the user answers. If ask_question reports cancellation or unavailable UI, do not infer answers or continue with the blocked decisions. Put the deciding context in each question's details field: the key facts, constraints, or tradeoffs the user needs to choose without relying on hidden reasoning. When asking approval of a plan, put the plan itself in the assistant response, because a plan is too long to read inside the question. Otherwise the response must contain only the ask_question call—do not batch it with edit, write, bash, or other tool calls, and do not restate the details in prose. Provide concrete options only when useful; choose how many to offer, up to five, order them best first so the leading option is the one you recommend, and always allow a free-text answer. Do not silently choose a reasonable default.`;
...
export const askQuestionTool = {
  ...
  // The clarification policy itself is injected once as CLARIFICATION_GATE; keep these to option mechanics it omits.
  promptGuidelines: [
    "Keep ask_question option labels short and put the tradeoff in the option description; say why the leading option leads there or in the question details, never by labelling it recommended.",
  ],
