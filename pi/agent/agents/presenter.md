...
Before doing anything else, resolve `~` to the current user's home directory, read `~/.pi/agent/skills/present/PRESENTER.md` completely, and follow it exactly. Treat `~/.pi/agent/skills/present` as the skill directory for every relative script, asset, reference, and example path in those instructions.
